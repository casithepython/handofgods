...
def default_info(info, player_id):
  return  "> **{display_name}:**\n> " \
          "Pantheon: {pantheon}\n> " \
          ...
          "Remaining Priest Channeling Power: {channel:.0f}".format(
            ...
            soldiers=db.get_army(player_id),
            functionaries=db.get_attribute(player_id, Attributes.FUNCTIONARIES),
            priests=db.get_attribute(player_id, Attributes.PRIESTS),
            attack=db.get_attribute(player_id, Attributes.ATTACK),
            defense=db.get_attribute(player_id, Attributes.DEFENSE),
            armor=db.get_attribute(player_id, Attributes.ARMOR),
            initiative=db.get_attribute(player_id, Attributes.INITIATIVE),
            power=db.get_attribute(player_id, Attributes.POWER),
            income=db.calculate_income(player_id),
            channel=db.get_channeling_power(player_id)
          )

def income_info(info, player_id):
  return  "**{display_name}\'s income:**\n" \
          ...
          "Passive population growth rate: {pop_growth_rate:.0%}/turn"\
            .format(
              display_name=info["display_name"],
              total_dp=db.get_attribute(player_id, Attributes.POWER),
              income_per_functional=db.get_attribute(player_id,Attributes.INCOME_PER_FUNCTIONAL),
              income_per_soldier=db.get_attribute(player_id, Attributes.INCOME_PER_SOLDIER),
              income_per_priest=db.get_attribute(player_id, Attributes.INCOME_PER_PRIEST),
              base_income_per_turn=db.get_attribute(player_id,Attributes.INCOME_PER_FUNCTIONAL) +
                                    db.get_attribute(player_id, Attributes.INCOME_PER_SOLDIER) +
                                    db.get_attribute(player_id, Attributes.INCOME_PER_PRIEST),
              bonus_income_per_functional=db.get_attribute(player_id, Attributes.BONUS_POWER_PER_FUNCTIONAL),
              bonus_income_per_soldier=db.get_attribute(player_id, Attributes.BONUS_POWER_PER_SOLDIER),
              bonus_income_per_priest=db.get_attribute(player_id, Attributes.BONUS_POWER_PER_PRIEST),
              priest_income_boost_rate=db.get_attribute(player_id, Attributes.PRIEST_INCOME_BOOST_RATE),
              priest_income_boost_capacity=db.get_attribute(player_id,Attributes.PRIEST_INCOME_BOOST_CAPACITY),
              total_boost_capacity=db.get_attribute(player_id,Attributes.PRIESTS)* db.get_attribute(player_id,Attributes.PRIEST_INCOME_BOOST_CAPACITY),
              total_income_per_turn=db.calculate_income(player_id),
              pop_growth_rate=db.get_attribute(player_id,Attributes.PASSIVE_POPULATION_GROWTH_RATE)
            )

def war_info(info, player_id):
  return  "**{name}'s army:**\n" \
          ...
          .format(
              ...
              soldiers= int(db.get_attribute(player_id,Attributes.SOLDIERS)),
              total_soldiers = int(db.get_army(player_id)),
              attackers=int(db.get_attribute(player_id,Attributes.ATTACK_ELIGIBLE_SOLDIERS)),
              attacks_per_turn=db.get_attribute(player_id,Attributes.ATTACKS_PER_TURN),
              attack=db.get_attribute(player_id,Attributes.ATTACK),
              defense=db.get_attribute(player_id, Attributes.DEFENSE),
              armor=db.get_attribute(player_id,Attributes.ARMOR),
              initiative=db.get_attribute(player_id,Attributes.INITIATIVE),
              soldier_cost=int(db.get_attribute(player_id,Attributes.SOLDIER_COST)),
              soldier_disband_cost=int(db.get_attribute(player_id,Attributes.SOLDIER_DISBAND_COST)),
              functionary_defense=db.get_attribute(player_id,Attributes.FUNCTIONARY_DEFENSE),
              functionary_armor=db.get_attribute(player_id,Attributes.FUNCTIONARY_ARMOR)
          )


def conversion_info(info, player_id):
  return  "**{name}'s conversion metrics:**\n" \
          ...
          "Max priest channeling per turn: {channeling:.0f} \n".format(
            name=info["display_name"],
            enemy_rate=int(db.get_attribute(player_id, Attributes.ENEMY_CONVERSION_RATE)),
            enemy_cost=int(db.get_attribute(player_id, Attributes.ENEMY_CONVERSION_COST)),
            priest_convert_rate=db.get_attribute(player_id, Attributes.ENEMY_PRIEST_CONVERSION_RATE),
            priest_convert_cost=db.get_attribute(player_id, Attributes.ENEMY_PRIEST_CONVERSION_COST),
            neutral_rate=db.get_attribute(player_id, Attributes.NEUTRAL_CONVERSION_RATE),
            neutral_cost=db.get_attribute(player_id, Attributes.NEUTRAL_CONVERSION_COST),
            priest_cost=db.get_attribute(player_id, Attributes.PRIEST_COST),
            channeling=int(db.get_attribute(player_id, Attributes.MAXIMUM_PRIEST_CHANNELING))
        )


def research_info(info, player_id):
  return  "**{name}'s research metrics:**\n" \
          ...
          "Research cost multiplier (increases proportional to population): {multiplier}".format(
            name=info["display_name"],
            inspiration_rate=int(db.get_attribute(player_id, Attributes.DIVINE_INSPIRATION_RATE)),
            inspiration_cost=int(db.get_attribute(player_id, Attributes.DIVINE_INSPIRATION_COST)),
            asleep_rate=db.get_attribute(player_id, Attributes.ASLEEP_REVELATION_RATE),
            asleep_cost=db.get_attribute(player_id, Attributes.ASLEEP_REVELATION_COST),
            awake_rate=db.get_attribute(player_id, Attributes.AWAKE_REVELATION_RATE),
            awake_cost=db.get_attribute(player_id, Attributes.AWAKE_REVELATION_COST),
            avatar_rate=db.get_attribute(player_id, Attributes.DIVINE_AVATAR_RATE),
            avatar_cost=int(db.get_attribute(player_id, Attributes.DIVINE_AVATAR_COST)),
            priest_bonus=db.get_attribute(player_id, Attributes.PRIEST_RESEARCH_BONUS),
...
def pretty_list(iterable):
  ...
  if len(lst) == 1:
    ...
    return ', '.join(map(lambda x: str(x), lst[:-1])) + ' and ' + str(lst[-1])